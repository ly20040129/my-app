...
def load_ai_agent():
    """加载AI代理并缓存,提升性能"""
    try:
        st.sidebar.info("正在初始化大模型。。。")
        llm = ChatZhipuAI(
            model='glm-4',
            zhipuai_api_key=api_key,
        )
        #test_response=llm.invoke("您好！很生气为你服务")

        st.sidebar.success("大模型得吃得吃")

        st.sidebar.info("加载道具ing。。。")

        #tools = load_tools(["llm-math", "ddg-search"], llm=llm)
        math_tool=load_tools(["llm-math"],llm=llm)
        search_tool=DuckDuckGoSearchRun(name="duckduckgo_search",description="搜东西来的")
        tools = math_tool + [search_tool] 
        st.sidebar.success("加载完毕，玩去吧你！")

        st.sidebar.info("创建煞笔ai代理ing。。。")
        ...
        st.sidebar.success("创建成功，就快要。。飞起来！！！")
        return agent
    except Exception as e:
        #st.error(f"初始化AI代理失败: {e}")
        st.sidebar.error(f"初始化过程详细错误: {str(e)}")
        st.sidebar.error(f"错误类型:{type(e).__name__}")
        import traceback
...
with st.sidebar:
    ...
    **功能特点：**
    ...
    st.markdown("---")
    st.caption("Powered by ZhipuAI GLM-4 & LangChain to 罗永")